...
public class BedCheckHelper{
	...
	public void checkForBed(World world, int x, int y, int z, int itr){
		if(webFull.getWebType() == EnumWebType.NORMAL){
			...
			for(Block block: SpiderQueenFixer.bedLogs) for(int i = -2; i <= 2; i++)
				for(int j = -2; j <= 2; j++) if(BlockHelper.getBlock(world, x + i, y, z + j) == block && (i == -2 || i == 2 || j == -2 || j == 2)) outlineBlocks++;
